...
if __name__ == "__main__":
    ...
    experiment_times = 1
    percent = 0.05
    file = open("./output/embedding.txt")  # 1693978个单词，每个单词128维，从第2行开始，每行是单词，以及对应128维的数字表示
    file_1 = open("./input/googlescholar.8area.venue.label.txt")  # venue以及对应类别label
    file_2 = open("./input/googlescholar.8area.author.label.txt")  # author以及对应类别label
    check_venue = {}
    check_author = {}

    for line in file_1:
        venue_label = line.strip().split(" ")
        check_venue[venue_label[0]] = int(venue_label[1])  # venue name, label
    for line in file_2:
        author_label = line.strip().split(" ")
        check_author[author_label[0]] = int(author_label[1])  # author name, label
    ...
    for line in file:
        ...
        if embed[0] in check_venue:
            venue_embed_dict[embed[0]] = []  # embed[0] = name
            for i in range(1, len(embed), 1):  # embed[i] = 128维信息
    ...
    for time in range(experiment_times):
        ...
        for author in authors:
            count += 1
            # print("one more author " + str(count))  # 多余代码
        ...
        # split input into training and testing  多余的代码
